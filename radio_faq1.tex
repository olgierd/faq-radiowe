...
Na początek najlepszym “starter kitem”, który pozwala na lokalną łączność na fonii w pasmach 2m i 70cm, jest mniej więcej coś takiego:
\begin{itemize}
 \item radio ręczne, na przykład Baofeng UV-82, lub w wypadku większej ilości dostępnej gotówki Wouxun
 \item kabel antenowy i końcówka/przejściówka pozwalająca na podłączenie go do radia (ręczne radiotelefony posiadają zazwyczaj złącze typu SMA -- nie mylić z RP-SMA używanym w urządzeniach Wi-Fi -- nie będzie pasować)
...
Zasięg, zależny od propagacji to bardzo rozległy temat. Każde z pasm zachowuje się w trochę inny sposób. Fale krótkie (od 3 do 30 MHz) pozwalają na łączności globalne (międzykontynentalne), gdyż odbijają się one od jonosfery i powracają na ziemię. 80 metrów to głównie łączności krajowe, pasmo to ''chodzi'' zazyczaj wieczorami i nocą. 40 m jest pasmem uniwersalnym, w dzień pozwalając na łączności lokalne, nocą często można na nim usłyszeć dalekie stacje z Azji i Ameryki. Podobnie zachowuje się 30 metrów. 20 metrów to pasmo głównie dzienne, umożliwiające dalekie łączności. Wyższe pasma, czyli 17, 15, 12 i 10 metrów to pasma zdecydowanie dzienne, pozwalające na bardzo dalekie łączności, ale silnie zależne od stanu jonosfery i aktywności słonecznej. Często przez kilka tygodni pozostają one ''zamknięte'' -- nie jest możliwe prowadzenie łączności poprzez odbicie od jonosfery. Poprzez nasłuch, wykorzystanie dx-cluster'a, sieci reverse beacon oraz dedykowane oprogramowanie możliwe jest określenie propagacji, czyli możliwego zasięgu na danym paśmie fal krótkich.

Fale ultrakrótkie (za wyjątkiem pasma 50 MHz w okresach bardzo wysokiej aktywności słonecznej) nie odbijają się od jonosfery, przez co ich zasięg jest zazwyczaj ograniczony do kontaktów lokalnych na fali przyziemnej (kilkadziesiąt km). Uzyskiwane dystanse są zależne od rodzaju i wysokości anten, ukształtowania terenu i mocy nadajnika -- nadając z gór, nie jest zaskoczeniem łączność na dystansie 200 kilometrów. Kilka lub kilkanaście razy do roku, możliwe jest nawiązywanie łączności nawet na kilkaset kilometrów. Powodowane jest to przez odpowiedni układ ciśnień i temperatur w troposferze -- są to jednak zjawiska i okresowe (zazwyczaj max. 2-3 dni). Stacje z kierunkowymi antenami, odpowiednio mocnymi nadajnikami i czułymi odbiornikami, są w stanie prowadzić łączności przez tak egzotyczne sposoby propagacji jak odbijanie sygnału od przelatujących samolotów czy śladów po wpadających w ziemską atmosferę meteorach.

Pasma mikrofalowe, czyli częstotliwości powyżej 1 GHz, propagują się podobnie jak fale ultrakrótkie -- aczkolwiek im wyższa częstotliwość, tym bardziej tłumiony jest sygnał, co przekłada się na mniejszy zasięg. Fabryczne urządzenia nadawczo-odbiorcze na pasma mikrofalowe nie są (z małymi wyjątkami) dostępne w sprzedaży, co sprawia że aktywność na tych pasmach jest zdecydowanie mniejsza niż na uprzednio wymienionych, pomimo iż oferują one ciekawe możliwości propagacyjne (np. odbicie sygnału od kropli deszczu w paśmie 3 cm).

\section{Co to w ogóle jest to „pasmo”?}
Pasmo to po prostu jasno określony zakres częstotliwości. Często podawana jest długość fali np. pasmo 144 MHz to inaczej pasmo 2 m. Długość fali o danej częstotliwości otrzymujemy dzieląc prędkość światła przez częstotliwość, np. $ 300\frac{mln. km}{s} / 24.9 MHz = 12.04 m $, czyli pasmo 12 metrów. Nazewnictwo jest zwyczajowe -- pasmo 14,000 -- 14,350 MHz nazywa się pasmem 20 metrów, mimo iż fala o długości 20 m to równe 15 MHz.

\section{Co to jest DX?}
DX to potoczne określenie dalekiej łączności. Na falach krótkich nawiązuje się je przez odbicie sygnału od jonosfery -- jednej z warstw atmosfery. Odbity sygnał trafia z powrotem na ziemię, kilkaset lub kilka tysięcy kilometrów dalej, odbija się ponownie i po kilku takich cyklach trafia do odbiorcy, który może znajdować się nawet kilkanaście tysięcy km od nas. Definicja DXa jest zależna od pasma -- nawiązanie łączności z Wielką Brytanią na falach krótkich to nic szczególnego, w paśmie 2m jest to natomiast rzadkie osiągnięcie. 
...
Przemiennik to, najprościej mówiąc, stacja retransmisyjna - odbierająca na jednej częstotliwości i nadająca na drugiej. W paśmie 2m jest to różnica 600kHz (np. odbieramy przemiennik na 145,600MHz, nadajemy do niego na 145,000MHz), a w paśmie 70cm - 7,6MHz. Różnica ta nazywana jest ‘shift’ lub ‘rpt offset’ i w Polsce oraz większości krajów jest ujemna, co oznacza, że powinniśmy w radiu słuchać częstotliwości wyjścia przemiennika, a nadawać na częstotliwości niższej. Aby skorzystać z przemiennika, należy go ``otworzyć'' czyli uruchomić. Zwykle robi się to na 3 sposoby:
...
Łączność należy utrzymywać przy minimalnej mocy niezbędnej do komfortowej pracy, oczywiście nie przekraczając wartości podanej na pozwoleniu. Lepiej zrobić dobrą antenę niż zwiększać przesadnie moc naszej stacji -- jeśli chcemy mocno nadawać, to warto też dobrze odbierać.
...
Anteny można podzielić do dookólne i kierunkowe. Im bardziej kierunkowa jest antena, tym ma większy zysk, bo więcej mocy jest wykorzystane do promieniowania fal radiowych w konkretnym kierunku. Dla fal ultrakrótkich najpopularniejszą anteną kierunkową jest antena typu Yagi-Uda, pozwalająca na zysk energerytyczny  w zakresie kilka--kilkanaście decybeli.
Policzmy -- różnica pomiędzy 100W a 400W, czyli czterokrotny zysk mocy, to o 6 decybeli mocniejszy sygnał odbierany (+6dB), czyli tylko 1 punkt na skali s-metra w radiu (mierniku siły odbieranego sygnału).

Zamiast używać nadajnika o większej mocy, możemy niewielkim kosztem zrobić antenę, która będzie miała zysk energetyczny. Zysk energetyczny jest to stosunek mocy promieniowania w danym kierunku naszej anteny do mocy anteny wzorcowej, którą najczęściej jest dipol (o zysku 0 dBd) lub antena izotropowa (nadająca równomienie we wszystkich kierunkach, o zysku 0 dBi). Zysk dipola jest większy w stosunku do anteny izotropowej o 2,15 dB więc: 0 dBd = 2,15 dBi. Antena z zyskiem 3 dBd emituje w danym kierunku 2 razy więcej energii niż dipol. W zależności od rodzaju anteny, można osiągnąć zysk energetyczny od kilku do nawet kilkudziesięciu dBd. W ten sposób dostarczając do anteny taką samą energię, możemy wyemitować dużo silniejszy sygnał w określonym kierunku -- ale słabszy w pozostałych kierunkach.
...
Wykorzystując własnoręcznie zbudowany sprzęt należy pamiętać o bezpieczeństwie (własnym i osób postronnych) oraz upewnić się, że wykonane przez nas urządzenia nie powodują zakłóceń i nie emitują niepożądanych sygnałów.
...
To zależy jaką antenę chcemy na nim zamontować. Dla anten pionowych nie będzie to przeszkoda, dla poziomych (np. dipol na pasmo 2m) warto zapewnić 1/4 fali odstępu od balustrady. Montując antenę na balkonie, należy starać się jak najmocniej odsunąć ją od ściany.

\section{Jaki jest koszt całej imprezy?}
W zależności od potrzeb. Można zacząć od zabawy w „nasłuchowca” kupując podłączany do komputera odbiornik „RTLSDR” za kilkadziesiąt złotych, można też zacząć od prostego radia ręcznego na pasma 2m i 70cm (z modulacją FM) w okolicach 200zł. Fale krótkie wymagają większego nakładu -- używany transceiver jedno- lub dwupasmowy DIY to 300-500 zł, używane fabryczne urządzenie na wszystkie pasma -- od 1000zł w zależności od stanu. Z tzw. allbanderami (urządzenie obsługujące kilka pasm amatorskich, zwykle z zakresu 160m-70cm) sytuacja jest już troszkę mniej kolorowa, gdyż za nowy sprzęt trzeba czasami wydać od 3, do nawet 30 tysięcy złotych za sprzęt dla najbardziej wymagających radioamatorów. Na szczęście w środowisku można znaleźć osoby, które sprzedają używany sprzęt za sensowne pieniądze.
...
Transmisje cyfrowe to transmisje nadawane i odbierane przy użyciu podłączonego do radia komputera (lub tabletu, telefonu) z odopwiednim oprogramowaniem (np. MixW, FLdigi). Najpopularniejsze emisje obecne na falach krótkich to PSK-31, RTTY (umożliwiają przesyłanie tekstu); JT65 (bardzo powolna transmisja -- nadanie 13 znaków trwa prawie minutę, ale przez to da się odebrać bardzo słabe sygnały i zaliczać dalekie łączności małą mocą i skromnymi antenami); SSTV - transmisja obrazów linia po linii w sposób analogowy).

Na falach ultrakrótkich najbardziej rozpowszechniona transmisja cyfrowa to APRS; do łączności troposferycznych i EME wykorzystuje się JT65.

Zaletą emisji cyfrowych jest wysoka sprawność energetyczna: pasmo ludzkiego głosu to 3 kHz, sygnał PSK-31 zajmuje tylko kilkadziesiąt Hz -- moc naszego nadajnika skupia się w wąskim wycinku częstotliwości, pozwalając na nawiązanie łączności nawet jeśli nie jest to możliwe na SSB. Dodatkową zaletą jest brak konieczności wydzierania się do mikrofonu, co pozytywnie wpływa na stosunki z domownikami (szczególnie późną nocą).

Każde pasmo fal krótkich ma wydzielony fragment do pracy emisjami cyfrowymi: okolice 3570, 7040, 10140, 14070, 18100, 21080, 24920, 28100 kiloherców -- szczegółowe informacje dostępne są w bandplanach. 
...
EME (Earth-Moon-Earth), to łączności polegające na odbijaniu sygnału od powierzchni księżyca. Wymagają one rozbudowanych anten (długie Yagi) i dużych mocy, by można było chociaż ułamek mocy usłyszeć znów na Ziemi. Najpopularniejsze pasma to (kolejno) 2m, 23cm, 70cm. Większość łączności to cyfrowe JT65 i telegrafia, stacje mające talerze o średnicy 15 metrów albo zestawy 16 długich anten Yagi-Uda mogą prowadzić też łączności SSB.
...
Można. Mało to ciekawe na dłuższą metę ale można. Częstotliwości łatwo znaleźć w sieci szukając po kodzie lotniska wg. ICAO (np. Warszawa Chopin = EPWA) i oznaczeniu częstotliwości: APP -- approach (podejście), GND -- ground, TWR -- tower.
